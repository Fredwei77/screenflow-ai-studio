...
echo "=== ScreenFlow AI Studio - Deploy to www.emai2.cn/meet ==="
echo ""
...
CORS_ORIGIN=https://www.emai2.cn
EOF
...
for conf in /etc/nginx/conf.d/*.conf /etc/nginx/sites-enabled/*; do
    if [ -f "$conf" ] && grep -q "www.emai2.cn" "$conf" 2>/dev/null; then
        SITE_CONF="$conf"
        break
    fi
done
...
if [ -z "$SITE_CONF" ]; then
    echo -e "${RED}Could not find nginx config for www.emai2.cn${NC}"
    echo "Please manually add the following location blocks to your nginx server config:"
    echo ""
    cat nginx/meet.emai2.cn.conf
    echo ""
else
...
\
    location /meet/ {\
        ...
        if nginx -t 2>/dev/null; then
            ...
            nginx -t
            echo ""
            echo "Please manually add the location blocks from nginx/meet.emai2.cn.conf"
        fi
    fi
fi
...
echo "============================================"
echo ""
echo "  App:     https://www.emai2.cn/meet"
echo "  Health:  curl -k https://www.emai2.cn/meet/api/health"
echo ""
